...
- [x] Cloud Schedulerの初回実行結果を確認（ニュース収集・取引実行）
- [x] GMO API 本番注文の動作確認（Real API テスト）
- [x] **[2026-05-31] バグ修正: ニュース signal が常に IGNORE になっていた問題を修正**（`news_storage._derive_signal()` 追加、Firestore 931件中 698件をバックフィルで修正）
- [x] **[2026-05-31] バグ修正: AIロットが常に1000固定になっていた問題を修正**（`determine_lot()` を v2.1 線形スケールに変更、100刻み 500〜1500）
