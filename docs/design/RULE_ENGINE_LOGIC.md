...
AIは**ファンダメンタルズ（ニュース分析結果）とテクニカル方向の一致度**を `avg_sentiment` の絶対値で評価し、500〜1500通貨を100刻みで線形決定する。

> **v2.0 → v2.1 変更理由**: 実データの `avg_sentiment` は -0.5〜+0.5 の範囲に集中していることが判明。v2.0 の 3段階閾値（0.5区切り）では実質的に 1000 固定になっていた。
...
if avg_sentiment < 0:
    ...
    ratio = min(1.0, avg_sentiment / 0.5)
    lot = round((500 + ratio * 1000) / 100) * 100  # 100刻み

# SELL シグナルの場合
if avg_sentiment > 0:
    lot = 0  # シグナルと反対方向 → 見送り
else:
    ratio = min(1.0, abs(avg_sentiment) / 0.5)
    lot = round((500 + ratio * 1000) / 100) * 100  # 100刻み
```
...
| |avg_sentiment| ロット |
|---|---|---|
| シグナルと反対方向 | BUY時 < 0 / SELL時 > 0 | **0（見送り）** |
| 完全中立 / ニュースなし | 0.0 | **500** |
| 弱い一致 | ±0.1 | **700** |
| 中程度の一致 | ±0.25 | **1,000** |
| 強い一致 | ±0.4 | **1,300** |
| 非常に強い一致 | ±0.5以上 | **1,500（上限）** |

**テクニカル = HOLD の場合:** ロット = 0（取引しない）

#### 入力データ

既存の `RuleEngine._summarize_news()` の出力をそのまま使用:

- `avg_sentiment`: -2 〜 +2（実運用では概ね -0.5〜+0.5）

#### GMOコイン注文単位

- GMOコイン FX APIの `size` パラメータ: **1 = 1通貨**
- 最小注文単位: 100通貨
- ロット範囲: 0（見送り）または 500〜1500（100刻み）
...
| v2.1 | 2026-05-31 | **バグ修正2件**: (1) `news_storage.py` でニュースの `signal` フィールドが常に `"IGNORE"` に固定されていたバグを修正（`_derive_signal()` を追加。既存931件中698件をバックフィルスクリプトで修正）。(2) `determine_lot()` の3段階閾値が実データ範囲（avg_sentiment 0〜0.5）でほぼ1000固定になっていたバグを修正（100刻み線形スケールに変更）。 |
