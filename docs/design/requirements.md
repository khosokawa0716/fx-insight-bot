...
#### 2.2.1 取引環境
- **取引業者**: GMOコイン（REST API）
- **対象通貨ペア**: USD/JPY（v2.0で単一ペアに変更）
- **口座資金**: 100,000円（v2.0で変更）
...
#### 2.2.3 リスク管理（v2.0 改訂）
- **1取引最大損失**: 700円（設計目標。最大ロット1500通貨×SL40pips=600円）
- **月間最大損失**: 5,000円（超過で新規取引停止）
- **ポジションサイジング**: AIがロット決定（0/500/1000/1500通貨）
- **ストップロス**: 40pips（固定）
- **テイクプロフィット**: 40pips（固定）
- **注文方式**: IFDOCOのみ
- **同一通貨ペア同時保有**: 1ポジション
...
| 2026-02-14 | 2.0.0 | v2.0仕様改訂: AIロット決定、IFDOCO固定、リスク管理全面改訂 |